...
- **Configuration**: `timeout=` (per call), `lager python --breakpoint-timeout <s>`, or `LAGER_BREAKPOINT_TIMEOUT` (env) override the auto-resume duration; `timeout=0` / `--breakpoint-timeout 0` waits indefinitely; `LAGER_BREAKPOINTS=off|0|false` makes every `pause()` a no-op (and `pause()` outside `lager python` — no `LAGER_PROCESS_ID` — is a safe no-op).
...
- **`lager diagnose` prints an actionable message when the box is unreachable** instead of wrapping the raw urllib3 traceback. The previous output read `Could not fetch net list from box: HTTPConnectionPool(host='100.82.82.45', port=5000): Max retries exceeded with url: /nets/list (Caused by NewConnectionError("HTTPConnection(host='100.82.82.45', port=5000): Failed to establish a new connection: [Errno 61] Connection refused"))`. Now reads: `Box 'PRD-1' unreachable at 100.82.82.45:5000 (connection refused). The lager container may be stopped. Check with: lager ssh --box PRD-1 -- "sudo docker ps"`. `requests.exceptions.ConnectionError` and `Timeout` are caught explicitly with tailored messages (refused vs timed out); other exceptions still fall through to the catch-all.
...
### Fixed
- **DA1469x flash hung at chunk 4 (`0x18000`) on JUL-5.** Two bugs in the freshly-ported pure-RPC loader path that didn't show up against the GDB-script reference flow:
