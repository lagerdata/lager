...
## How the checks drive it

`lager uart --interactive` requires BOTH stdin and stdout to be terminals
(`cli/commands/communication/uart.py:512`), so the commands cannot be piped in
and the output cannot be piped to `tee`. Section 15 goes through
`test/framework/uart_pty_driver.py`, which allocates a pty, waits for the
session's "Connected to" line before typing, paces each command on output
silence, and stops the session with SIGINT **to the process group** — `lager`
is a pyenv shim that execs through `pyenv exec`, and signalling a single pid in
that chain does not reliably reach the Python process.
