...
| `test_flash_programming_verdict.py` | A J-Link flash that programmed nothing is not reported as done. `flash_device()` returns the failing line as its generator return value, judged on the flash session's own output (a failed or unverified RAMCode download, `Failed to prepare for programming.`, `Error while determining flash info`, a failed attach) and never on `Downloading file`, which J-Link prints first. Also that J-Link's RAMCode for memory access or FPU registers, a signature mid-line, and a verify failure are not verdicts. A DA1469x whose programming failed skips the post-flash reset and says so, while other targets still reconnect the gdbserver. `/debug/flash` reports `programmed` / `error` as a 200 on the J-Link path and `programmed: true` on OpenOCD. A failed flash reports what else was going on: any other J-Link process on the probe (read from `/proc`, taken before the post-flash steps start a client of their own) and, on a DA1469x, whether and how the target reset during programming (`RESET_STAT_REG`, each cause named); a clean flash runs no diagnosis, and an unreadable register does not mask the failure. A Commander session that cannot use the probe (`Selected interface (SWD) is not supported by the connected probe.`, `Target connection not established yet ...`) fails erase and flash, skips the reset and is diagnosed, and a `Downloading file` with no `Flash download` line before the next one fails the flash, a skipped already-matching bank counting as a download. The probe-driving endpoints (`connect`, `disconnect`, `reset`, `flash`, `erase`, `memrd`, not `rtt`) run whole under their probe's lock, a net that does not resolve still reaches its handler, and a busy probe answers 503 naming the holder. Success needs evidence: a J-Link flash with no `loadfile` at all (silent Commander output, as a probe dropping off USB leaves) or a `JLinkExe exited` line fails, a bare `O.K.` after `Downloading file` counts and an API-trace `O.K.` does not; an erase needs `Erasing done.` or a range confirmation. The other-client scan compares serials as numbers (`50115930` is `000050115930`), counts a client that names no probe, leaves out this process's own JLinkExe, and a sampler keeps clients seen during the flash; the `RESET_STAT_REG` read is retried. The lock key is the probe serial; an OpenOCD net with no serial keys on its address so two such probes stay independent, and a J-Link net with no serial stays on `default` |
...
| `test_probe_lock.py` | One J-Link operation per probe at a time, since J-Link lets a second client interleave with a flash and corrupt its RAMCode download: a second thread or process waits for the first, the wait gives up with `ProbeBusyError` naming the holder, the lock is re-entrant within a thread, a holder that dies releases it, separate probes never block each other, and `holds_probe` holds a generator's lock for its whole life (released on close) while keeping its return value; a thread that waits behind another logs it, naming the holder |
