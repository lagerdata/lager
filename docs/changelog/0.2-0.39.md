# Changelog archive: 0.2.17 to 0.39.1

Releases 0.40.0 and later are in [CHANGELOG.md](../../CHANGELOG.md). The sections below moved here from CHANGELOG.md, so that CHANGELOG.md stays small enough for GitHub to render.
...
Interactive breakpoints for `lager python` scripts. A long-running test can now pause itself mid-run so the operator can inspect the bench with ad-hoc `lager` commands (or a live Python prompt) and then continue — for debugging tests against a device in an unknown state. Before this, `lager python` scripts ran start-to-finish with `stdin` set to `DEVNULL` and there was no way to hold execution at a chosen point.
...
This release responds to a field report of a battery net not responding on a Keithley 2281S. The biggest items below — `lager diagnose`, the `usbtmc` blacklist, automatic ENODEV recovery, and cross-process device locks — collectively eliminate the most common USB instrument failure modes, and surface the rest (e.g. wedged instrument firmware that only mains-power-cycling can fix) with a single one-line diagnosis.
...
- **Documented "TUIs are laptop-only"** in `box/lager/README.md`. A TUI run directly on the box is a second `pyvisa-py` client competing with hardware-service for interface 0. The OS-level `device_lock` makes this case detect-and-fail-clean instead of silent EBUSY, but the right answer is still: always launch TUIs from the laptop CLI.
