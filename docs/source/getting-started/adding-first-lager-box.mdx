...
- A Lager Box -- see [Setting Up a Lager Box](/source/getting-started/setting-up-a-lager-box) if you have not set one up yet
...
**Expected output:** the version number alone, for example:
```
0.47.0
```
...
The command below lists every Lager Box you added.
...
If the last line says that your Lager Box is online, you are ready to start using it!
The value in brackets names the ref that the box runs. See
[`lager hello`](/source/reference/cli/hello) for the other forms of that line.
...
| `Connection refused` | The Lager Box service is not running, or the box was never set up | The box may need a restart or a software update. Contact your administrator, or see [Setting Up a Lager Box](/source/getting-started/setting-up-a-lager-box). |
