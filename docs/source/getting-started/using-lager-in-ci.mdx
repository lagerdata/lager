...
- In a CI job that the CLI detects, it runs the command directly. The CLI expects
  the job to run in the image already, so there is nothing to start. A job
  container also has no Docker.

The CLI detects the job from the CI variables only. It does not check for a
container. Give the image to the job with `container:`:
...
Every job on GitHub Actions, GitLab CI, Drone, and Bitbucket Pipelines gets the
second behavior. This includes a job with no `container:`. In that job, the command
runs on the runner itself, not in the image. A Jenkins agent and other CI systems
...
To start a container from a CI job, set `LAGER_CI_OVERRIDE` to any value that is not
empty. The job must have Docker. Set the variable only on the `lager exec` step:
...
<Warning>
`LAGER_CI_OVERRIDE` stops CI detection for each `lager` command that sees it. A
lock collision then gives an error at once, and the lock holder is the user name.
Do not set it for the whole job.
</Warning>
...
`LAGER_CI_OVERRIDE` is not set. A CLI older than 0.46.2 always starts a container.
Update the CLI. See [section 6](#build-with-lager-exec).
