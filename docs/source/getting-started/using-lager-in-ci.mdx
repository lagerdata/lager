...
`lager boxes unlock` sends the user name, not the CI identity. The box releases
a lock only for the same holder. Thus this step cannot release an automatic
`ci:` lock. It releases only a lock that the user name of the runner account
holds, such as a `lager boxes lock` reservation:
...
<Warning>
A `lager boxes lock` reservation of the runner account also blocks the CI
commands of that account. The CI identity is not the same as the user name. A
CI command then waits for `LAGER_LOCK_WAIT` seconds and gives exit code 1.
</Warning>
...
| Variable | Function |
|---|---|
| `LAGER_LOCK_WAIT` | Seconds to wait after a collision. CI default 1800. User default 0. A value that is not a number gives 0, also in CI. A negative value gives 0. |
...
| 1 | An update is necessary: the code, the dependencies, or the container. Exit code 1 also shows that the check failed. |
| 2 | The CLI did not confirm an SSH key for the box, or `git rev-list` failed on the box. |

Exit code 1 does not always tell you that the box is old. These failures also
give exit code 1:
...
| `LAGER_LOCK_HOLDER` | A different identity for the lock holder. |
| `LAGER_LOCK_WAIT` | Seconds to wait after a lock collision. A value that is not a number, or a negative value, gives 0. |
...
| | 130 | A person or a signal stopped the command. |
| | 128+N | Signal N stopped the script on the box. |
| `lager update --check` | 0 / 1 / 2 | Correct / an update is necessary, or the check failed / no confirmed SSH key, or `git rev-list` failed. |
...
holder. Do not use `unlock --force` in a job. If the holder is the user name of
the runner account, a `lager boxes lock` reservation blocks the job.
