...
description: Run hardware tests from GitHub Actions with the runner installed on the Lager Box
---
...
network. The job must install the CLI, get a credential, and get the address of
the box.

If your boxes sit behind an access gateway, give the job a token. Your auth
server makes the token. The CLI reads it from `LAGER_GATEWAY_TOKEN` and sends it
with each request. The job then needs no login step and no password:
...
Some auth servers cannot make a token for a machine. For those, the job logs in
as a person:
...
Boxes with no gateway need neither form. Remove the credential from the job.
[Section 13](#13-how-to-give-ci-a-gateway-credential) gives the detail of both
forms.
...
- The address of the box becomes a repository secret. The login form adds an
  email address and a password. The token form adds one token.
...
and the `PATH` on your first box. Then use these steps on all the boxes.
</Note>
...
<Warning>
Do not give two boxes the same label. If two boxes have the same label, a job
can go to the wrong bench. That bench can lack the nets that the job needs.
...
`lager install` and `lager update` put the CLI on the box. The command is
`~/.local/bin/lager`. It is a link to a separate Python environment at
`~/.lager_venv`. The CLI has the same version as the box code. Each install,
and each update that deploys code, writes the link again.
...
- **Use Python 3.10 or a subsequent version on the box.** The CLI needs it.
- **Use the CLI that the box installs.** Do not use `pip install --user` on the
  box. On Ubuntu 23.04 and later versions, that command fails, because the
  system Python is externally managed. A second CLI that you install in a
  different location does not follow the version of the box.
- **Change the version with `lager update`.** The update also installs the CLI
  of the new version. The CLI compares its version with the version of the box
  with each command. It gives a warning if they are different. The command
  `lager boxes` shows each box as `current`, `needs update`, or `newer`.
...
[Section 13](#13-how-to-give-ci-a-gateway-credential).
...
<Warning>
Use `cancel-in-progress: true` only if you also clean the bench.
...
Each `lager` command has a different `<pid>`. To compare two holders, the CLI
removes the `:<pid>` part. Thus a later command in the same job accepts the
automatic lock of that job as its own. The remainder of the text keeps the run,
the attempt, the job name, and the runner. `lager boxes` shows the identity in a
...
<Warning>
A `lager` command on the same box that does not operate in an
...
An automatic lock ends when its command exits. If the CLI stops in an unusual
condition, the lock ends at its maximum life.

The box releases a lock only for the same holder string. Thus
`lager boxes unlock` reads the lock first. The lock can be an automatic lock of
the same job, or a `lager boxes lock` reservation of the runner account. For
these locks, unlock sends the holder string that the box stored. Thus this step
releases both:
...
A `lager boxes lock` reservation of the runner account does not block the CI
commands of that account. A CI command uses the reservation and does not
release it.

A generic CI provider gives all jobs on one host the same scope. For those
jobs, unlock releases only a lock with the exact holder string.

<Warning>
Do not use `lager boxes unlock --force` in a job. This command
...
| Variable | Function |
|---|---|
| `LAGER_LOCK_WAIT` | Seconds to wait after a collision. CI default 1800. User default 0. A value that is not a whole number gives a warning and the default. A negative value gives 0. |
...
| `LAGER_AUTO_LOCK_DISABLE` | Any value that is not empty stops the automatic lock. `0` also stops it. |
...
lock until the detached job ends. This is after your workflow step ends. A box
older than 0.42.0 does not take the lock. The lock then stays after the job
ends. Release it with `lager boxes unlock`.
</Note>
...
### Build with `lager exec`

If the build commands are in your `.lager` file, `lager exec` runs them.

`lager exec` has two behaviors:

- On a developer machine, it starts a container from the DEVENV `image`.
- In a CI job that runs inside a container, it runs the command directly. The
  job is in the image already, so there is nothing to start. A job container
  also has no Docker.

The CLI reads the CI variables, then checks whether this process is in a
container. Both have to be true. Give the image to the job with `container:`:
...
A job on GitHub Actions, GitLab CI, Drone, or Bitbucket Pipelines gets the second
behavior. It gets that behavior only when it runs in a container. A job with no
`container:` runs on the runner. The CLI then starts a container there and prints
one line. A Jenkins agent and other CI systems get the first behavior.

The command runs in the directory of the check-out. It does not run in `mount_dir`,
because there is no bind-mount.

These options do nothing when the command runs directly: `--mount`, `--volume`,
`--user`, and `--group`. If you give any of them, the CLI prints one warning line
that names them. These `.lager` keys also do nothing: `network`, `ports`, `platform`,
`macaddr`, `hostname`, `volumes`, `user`, and `group`. The CLI names these keys only
with `--verbose`. The `--env` option and the `environment` key operate as usual.

To choose the path yourself, set `LAGER_EXEC_IN_PLACE`. Set it to `1` to run the
command in place, and to `0` to start a container. The job must have Docker for
`0` to work:

```yaml
- run: lager exec build-ci
  env:
    LAGER_EXEC_IN_PLACE: "1"
```

`LAGER_EXEC_IN_PLACE` changes `lager exec` and nothing else. Box locking is the
same with it set.

<Warning>
`LAGER_CI_OVERRIDE` also starts a container, and it costs more. It stops CI
detection for each `lager` command that sees it. A lock collision then gives an
error at once, and the lock holder is the user name. Prefer
`LAGER_EXEC_IN_PLACE=0`, and do not set `LAGER_CI_OVERRIDE` for the whole job.
</Warning>
...
`lager debug flash` and `lager debug erase` read the programmer output. They give
exit code 1 when a line starts or ends with one of these texts:
...
The command then prints `Flash failed:`, `Flash erase failed:`, or
`Erase failed:` with that line. Before the operation, the connection check to the
target can also give exit code 1. CLI versions before 0.42.0 do not do these
checks.

With other failure text, the exit code stays 0. The device can then be erased
but not programmed. The tests fail subsequently, and the cause is not clear.
Record the output and look for failure text that the CLI does not recognize.
...
Sometimes it is better. The test that programs the device is also the test that
shows the program operation is correct.

<Note>
The Python `DebugNet` methods give the output of the programmer as
...
| 2 | The CLI did not determine the state of the box. |

Exit code 1 has one meaning: the box needs an update. Each failure below leaves
the state of the box unknown, so each one gives exit code 2:

- An SSH connection that times out, or another SSH error.
- A box that does not answer the inspection over SSH.
- A box directory that is not a git checkout.
- A `git fetch` on the box that fails. A commit SHA that no branch or tag on
  GitHub contains also gives this failure.
- A box that a different holder locked.
- A box with no SSH key that the CLI confirms, or a `git rev-list` that fails
  on the box.

When an update is necessary, the output ends with
`Run without --check to apply.` The example script uses this text. Exit code 2
gives you no data about the box.

<Note>
CLI versions before 0.49.0 give exit code 1 for these failures too. The example
script matches on `Run without --check to apply.` for that reason. It is
correct against the old codes and the new ones.
</Note>

`--check` shows the version of the box in its `Current:` line. It does not show
the git reference. To see the reference and the commit, use `lager hello`. Its
`Version:` line has the reference and the commit in parentheses.
...
An enable signal can stay high.

The next job gets this condition. The next person at the bench also gets it.
...
path open until a session sets them. Then the DUT has no power.

A DUT with no power gives this message at the **flash** step: "cannot connect
to the target". This message looks like a debugger failure. With a separate
...
- **Set the equipment to a safe condition. Do not release the lock.** The job
  ...
  holds it.
- **Do not use privileges.** The runner account has little sudo permission, or
...
| Exit code | Meaning |
|---|---|
| 124 | The `--timeout` time ended. The box sent SIGTERM. |
| 137 | The `--timeout` time ended. The box sent SIGKILL 5 seconds later. |
| 255 | The CLI did not get the exit code from the box. |
| 130 | A person or a signal stopped the command. |
| 128+N | Signal N stopped the script on the box. |
...
1. **For a role with more than one net, put the role first and the function
   second.** Examples are `USB_CHARGE`, `USB_DEBUG`, `UART_CONSOLE`, `ADC_VBUS`
   and `GPIO_NRST`. The name and the role must agree. Then you can find an
   incorrect connection.
2. **Give the power nets the name of the instrument, not the function.** Use
   `BATT` for a battery-simulator net and `SUPPLY` for a programmable-supply
   net. Then the role and the name give the same data on purpose.
3. **For a role with only one net, use the bare name.** Use `SWD` for the one
   debug probe and `UART` for the one console.
...
```bash
lager nets --box BENCH-1                    # list (no `list` subcommand)
...
<Note>
The command is `lager nets` with an `s`. The subcommand is `delete`,
...
## 13. How to give CI a gateway credential

Boxes behind an access gateway need a credential. Boxes with no gateway need
none. If your boxes have no gateway, do not read this section.

There are two credentials, and the CLI takes the first one it finds:

1. A token in `LAGER_GATEWAY_TOKEN`. Use this when the runner is not on the box.
2. A session from `lager login`. Use this when the runner is on the box.

### A token from your auth server

Ask your auth server for a machine token. Keep the token in a repository secret.
Then give the secret to the job:
...
This is all that the job needs. There is no login step, and no password goes
into your repository secrets. Each box action belongs to the token, not to a
person.

The CLI sends this token with each request to a box. It does not refresh the
token, and it writes no session file. Thus the job leaves no credential on the
runner.
This matters on a self-hosted runner, because the disk of the runner stays
between two jobs.

An empty value counts as no token. A secret that does not exist expands to an
empty string. The CLI then behaves as if you set no variable.

If the gateway refuses the token, the command stops immediately. The CLI does
not try a second credential. The message names the auth server that refused the
token. Get a new token from that server, then set the secret again.

### A session from `lager login`
...
machine with no token, or a box that you install again at intervals. For these
conditions, use the flags:
...
<Warning>
Read the password from a secret. Do not write the password in the
...
- **A token in `LAGER_GATEWAY_TOKEN` wins over a session.** The CLI reads the
  variable first and sends that token. It does not touch the session file. The
  Rust SDK reads the same variable.
...
A token in `LAGER_GATEWAY_TOKEN` does not have this condition. The CLI sends the
token with the first command, so there is no mapping to record.
...
| Variable | Function |
|---|---|
| `LAGER_BOX` | The default box when there is no `--box` flag. |
...
| `LAGER_GATEWAY_TOKEN` | A token for an access gateway. It wins over a session, and the CLI writes no file. |
| `LAGER_USER` | The user identity. It is the lock holder on a user computer. |
| `LAGER_LOCK_HOLDER` | A different identity for the lock holder. |
| `LAGER_LOCK_WAIT` | Seconds to wait after a lock collision. A negative value gives 0. A value that is not a whole number gives a warning and the default. |
| `LAGER_LOCK_TTL` | Maximum life of the lock. Use `none` for no limit. |
| `LAGER_LOCK_HEARTBEAT` | Seconds between two heartbeats. Default 60. |
| `LAGER_AUTO_LOCK_DISABLE` | Any value that is not empty stops the automatic lock, also `0`. |
...
| `LAGER_CI_OVERRIDE` | Any value that is not empty turns off CI detection. `lager exec` then starts a container, and the lock uses the user behavior. |
| `LAGER_EXEC_IN_PLACE` | `1` runs the `lager exec` command in place, `0` starts a container. It changes no other command and no lock. |

`lager python` sends these variables **to** your script on the box:
`LAGER_RUNNABLE`, `LAGER_PROCESS_ID` and `LAGER_OUTPUT_CHANNEL`. It sends
`LAGER_BOX` only when the command has the `--box` flag. A `LAGER_BOX` in the job
environment does not reach the script.
...
| | 124 | The `--timeout` time ended. The box sent SIGTERM. |
| | 137 | The `--timeout` time ended. The box sent SIGKILL. |
| | 255 | The CLI did not get the exit code from the box. |
| | 130 | A person or a signal stopped the command. |
| | 128+N | Signal N stopped the script on the box. |
| `lager update --check` | 0 / 1 / 2 | Correct / an update is necessary / the CLI did not determine the state of the box. |
| `lager exec` | the code of the command | The CLI does not change it. The command is in a container, or in the job, as [section 6](#build-with-lager-exec) shows. |
...
| Any lock flag: `--lock`, `--lock-wait`, `--no-lock` | The lock is automatic. Use the `LAGER_LOCK_*` variables. |
...
The defaults are `--interactive` and `--tty`. Give `--no-tty` in CI. This applies when
`lager exec` starts a container. When the CLI detects the CI job and runs the command
directly, it does not use these two options.

**`lager exec` gives the error `Docker is not installed or not in PATH`.**
The job is in a container, but the CLI tried to start one. Make sure that
`LAGER_CI_OVERRIDE` is not set, and that `LAGER_EXEC_IN_PLACE` is not `0`. If
the container hides the usual markers, set `LAGER_EXEC_IN_PLACE=1`. A CLI older
than 0.46.2 always starts a container, so update the CLI.
See [section 6](#build-with-lager-exec).

**`lager exec` starts a container where it used to run the command in place.**
The job declares no `container:`, so the command ran on the runner, not in the
image. The CLI now says so and uses Docker. Add `container:` to the
job, or set `LAGER_EXEC_IN_PLACE=1` to keep the old behavior.
...
The programmer printed failure text that the CLI does not recognize, and the
command gave exit code 0. The device is erased. Look for the failure text in the
output of the flash command. Refer to
[Section 6](#6-how-to-put-the-firmware-on-the-dut).
