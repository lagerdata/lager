...
flowchart TB
  subgraph container["Docker container: lager"]
    ...
    subgraph hs[":8080 hardware service"]
      direction TB
      DISP["Dispatchers<br/>BaseDispatcher subclasses"]
      CACHE["NetsCache<br/>mtime-invalidated, O(1) by net name"]
      DRV["Drivers<br/>VISA/SCPI, pySerial, LJM, pyOCD, aardvark_py"]
      DISP -->|"resolve net name"| CACHE
      DISP -->|"instantiate, cache, lock per device"| DRV
    end

    P9000 -->|"POST /invoke, Device proxy"| DISP
    P5000 -->|"spawns"| SUB
  end
  ...
  DRV --> INST
  SUB -->|"imports lager.*, drives drivers in-process"| INST
...
sequenceDiagram
    ...
    H->>D: Call the role adapter
    D->>D: Resolve psu1 through NetsCache
    D->>D: Instantiate and cache the driver
    D->>I: SCPI command
    I-->>D: Response
    D-->>H: Result
    H-->>B: Result JSON
