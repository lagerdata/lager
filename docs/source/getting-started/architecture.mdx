...
A PicoScope is the one instrument that goes over `/invoke` without holding a VISA session: it is
a USB device with a vendor SDK, so the hardware service reaches it by sending commands to the
oscilloscope daemon on loopback rather than by opening a VISA resource. The daemon owns the
device the way the hardware service owns a VISA session, and for the same reason — the PicoTech
SDK admits one process per unit.
...
| 8085 | TCP / WebSocket | No (loopback only) | Oscilloscope daemon control plane. Reached from outside through the port 9000 relay, never directly |
