...
| **Lagerbox** | Any x86-64 machine running Ubuntu 22.04 or newer, physically co-located with the test instruments. Runs a Docker container hosting the Flask/WebSocket server and hardware drivers. |
...
every service still listens inside the container and is reachable on the `lagernet` Docker
network, where a reverse proxy owns the host ports, but no `Yes` row is reachable at
`<box-ip>:<port>`. Port 22 is the exception -- SSH is the host's own daemon, not a published
container port, so it is unaffected.

---

## Optional Control Plane Integration

Lager boxes publish SSH keys from a key directory, `/etc/lager/authorized_keys.d/`, so an external control plane can provision access without a human typing SSH commands. Dropping a `<name>.pub` file there gets the key into the box account's `~/.ssh/authorized_keys` within about five seconds. Because `/etc/lager` is bind-mounted into the runtime container, a control plane can write that file from inside the container — which is how it bootstraps before it has any SSH access to the box at all.

`start_box.sh` owns only the region of `authorized_keys` between its `# BEGIN LAGER MANAGED KEYS` and `# END LAGER MANAGED KEYS` markers, and rebuilds that region from the key directory on every pass. Two consequences worth knowing:
...
- **Keys installed by other means are untouched.** `ssh-copy-id` and cloud-init append outside the marked region, and are preserved verbatim. Any other system that manages this file must claim its own distinct marker pair — two managers sharing one pair would each rebuild the other's region on every pass.
- **Preserved is not the same as durable.** `start_box.sh` preserves a loose line against *its own* rebuild; it cannot preserve it against someone else's. A second key manager that rebuilds `authorized_keys` from its own source, keeping only its own marked region, drops every loose line — and `start_box.sh` then re-creates its region from the key directory alone, so a key that was never registered there does not come back. For that reason `lager ssh-setup`, `lager update`, and `lager install` write their public key into the key directory as well as appending it, under `lager-box-<user>-<host>.pub`. Any tooling that installs a key it expects to survive should do the same.
...
```
GITHUB CLOUD                         TAILSCALE NETWORK           HARDWARE BENCH
============                         =================           ==============
...
jobs:
  hardware-test:
    ...
    steps:
      ...
      - name: Connect to hardware bench via Tailscale
