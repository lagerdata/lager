...
flowchart TB
  subgraph container["Docker container: lager"]
    ...
    subgraph hs[":8080 hardware service"]
      direction TB
      DRV["Drivers<br/>VISA/SCPI, pySerial, LJM, aardvark_py"]
...
|------|---------|---------|---------|
| 9000 | Flask + SocketIO | Yes | Main box API: UART streaming, live supply/battery WebSockets, instrument discovery, net listing, box lock. Not published when `LAGER_DISABLE_UART_SERVICE` is set |
| 5000 | HTTP | Yes | Python execution service: receives uploaded scripts and runs them as subprocesses |
| 8301 | HTTP | Yes | Alias of port 5000, kept for backward compatibility |
| 8765 | WebSocket | Yes | Debug sessions (GDB, flash, reset) |
| 8100 | HTTP | Yes | MCP service for AI tool integration. Not published when `LAGER_MCP_NO_PUBLISH` is set |
| 8080 | Flask | Yes | Hardware service: instrument control via the Device proxy, called by the port 9000 API |
...
| 8086-8090 | HTTP | Yes (if webcams present) | Webcam MJPEG streaming (one port per camera, from 8086) |
| 2331-2342 | TCP | Yes | Debug probe GDB, SWO and telnet ports, three per probe |
| 4444-4447 | TCP | Yes | OpenOCD telnet, one per probe |
| 6666-6669 | TCP | Yes | OpenOCD TCL, one per probe |
| 9090-9097 | TCP | Yes | RTT, two per probe |
| 22 | SSH | Yes | Direct SSH access for deployment and debugging |

<Warning>
  **The host firewall does not filter a published port.** Docker installs its forwarding rules
  ahead of the host chain. A published port therefore answers anyone who can route to the box,
  whatever `ufw status` reports. Treat network reachability as the boundary. Put the box on a
...
<Note>
  ...
  A box set to `lager box-config network-mode host` also publishes no ports. There, the
  container binds the ports on the host directly, and the host firewall governs them.
</Note>
...
Make both reachable over your VPN. A box that publishes only `:9000` answers
`lager nets` and `lager hello` but fails `lager python`.
...
| `debug` | J-Link (J-Link GDB server); ST-Link, CMSIS-DAP and FTDI debug adapters (OpenOCD) |
