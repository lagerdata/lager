...
`<box-ip>:<port>`. Port 22 is the exception. SSH is the host's own daemon rather than a
published container port, so `--no-publish` does not affect it.
...
A CI runner is an ephemeral GitHub Actions VM. It joins Tailscale and runs `lager` commands exactly like a developer. Lager needs no CI-specific infrastructure.
...
| `LAGER_BOX_IP` | Tailscale IP of the Lager Box (e.g., `100.x.y.z`) |
