...
30 minutes by default. Slower hardware runs longer. An emulated guest, a low-power mini PC,
or a throttled VM can exceed 30 minutes on a healthy build. Raise the budget with
`--timeout <seconds>` or `LAGER_INSTALL_TIMEOUT`, so the build does not stop midway.
</Note>
...
report `x86_64`. On any other architecture, an install can appear to succeed while hardware
support is missing.
