...
- **`git`** installed. Lager installs everything else the box needs, including Docker.
  Step 1 explains why you can install Docker yourself first. That step is optional.
...
<Note>
An install of the default `main` builds the Lager container on the box itself. That takes
roughly 14 minutes on ordinary box hardware. Plan for the box to be busy. `lager install`
bounds the step at 30 minutes by default. An install of a release tag with `--version` uses
the pre-built image instead, and takes about 2 minutes.
...
`--version v0.39.0`. A release tag uses the pre-built image, so the install runs much
faster. The default `main` builds on the box. See the
[Install reference](/source/reference/cli/install) for every option.
</Note>
...
Installation configures a UFW firewall on the box. The rules deny incoming traffic by
default. They allow the Lager service ports on `lo`, `docker0`, `tailscale0` and a
corporate VPN interface. They deny those ports on every other interface. Port 22 always
stays open, so the firewall cannot lock you out of the machine.

<Warning>
**On a default box, the firewall does not protect the Lager ports.** The container
publishes its ports through Docker. Docker forwards that traffic ahead of the host firewall
rules. A published port therefore answers anyone who can route to the box. Put the box on
a VPN or an isolated LAN. See the Security Model section of
[SECURITY.md](https://github.com/lagerdata/lager/blob/main/SECURITY.md#security-model).
</Warning>

The rules govern the Lager ports only on a box set to `lager box-config network-mode host`.
There, the container does not publish ports.
...
`tailscale0` interface and allows the Lager ports on it. The installer checks for Tailscale
only while it runs. `lager update` does not change the firewall.
...
The installer does not show this prompt when you pass `--yes`. The name is yours to choose
and is local to your computer. If you skipped that prompt, add it by hand -- `--user` is
required:
...
The value in brackets names the ref that the box runs. A default install deploys `main`, so
the line says `not a release build`. A box installed from a release tag shows only the tag
and the commit.
...
Pull the pre-built image instead of building it. Install a release tag rather than `main`,
so that a published image exists. `lager install` pulls it by default:

```bash
lager install --ip <box-ip> --user <box-user> --version <release-tag>
```

`lager update` builds on the box by default. For a later update, pass `--pull`:
...
branch, or a full 40-character commit SHA. Only a release tag has a published image to pull.
...
An install of a branch builds the container image on the box. That is the longest step by
far, and it can run for many minutes with little output. Let it run.

The command gives up when the `--timeout` budget runs out. The default is 1800 seconds, or
the value of `LAGER_INSTALL_TIMEOUT`. On slow hardware, raise the budget. `--timeout 0`
removes the limit. After a timeout, a re-run of the same command is safe.
...
On CLI 0.41.0 or newer, the installer validates the file before it installs it. A failure
prints `[ERROR] Invalid sudoers syntax -- /etc/sudoers.d/lagerdata-udev was NOT modified`.
The sudoers file on the box stays unchanged. On a `sudo-rs` box, the installer then prints
the `update-alternatives` command below. Report that output as a bug.
...
Each Docker install step reports its own failure. Look for a line like
`[lager] STEP FAILED: <command> (exit N)`. The lines just before it are the error output of
that command. If no such line appears, the SSH session itself failed, and the error above
comes from `ssh`.

If Docker is already on the box, the failure came after apt, in the daemon or the group
change. A second install does not help. The installer then suggests these commands on the
box:
...
Installation also prints the manual commands to run on the box:
