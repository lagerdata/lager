...
| Aardvark I2C/SPI | 专用的 USB SPI 适配器，采用 GPIO 位操作 |
| FTDI FT232H / FT2232H / FT4232H | 基于 MPSSE 的 SPI；通道按 Net 逐个选择 |

### 多通道 FTDI 适配器

FTDI Net 从 Net 记录的 `params.interface` 取得通道，接受 `A`-`D` 或 `0`-`3`。没有 `interface` 的 Net 使用通道 A —— 在单通道的 FT232H 上这也是唯一的选择。
...
SPI 运行在 FTDI 芯片的 MPSSE 引擎上，而在 FT4232H 上只有通道 A 和 B 带有 MPSSE 引擎。请求 C 或 D 会在 Net 构建时失败并指出该通道，而不是在 pyftdi 内部某处失败。
...
- Aardvark 采用 GPIO 位操作模式；无论 `frequency_hz` 设为多少，实际速度都受 USB 往返时间限制
