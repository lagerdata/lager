...
| `parity` | `str` | 校验位：`'none'`、`'even'`、`'odd'`、`'mark'` 或 `'space'`，或 pyserial 的 `'N'`、`'E'`、`'O'`、`'M'` 或 `'S'` |
| `stopbits` | `float` | 停止位（1、1.5 或 2） |
| `timeout` | `float` | 读取超时，单位为秒（默认 0.1）。`None` 表示一直等到有数据到达。 |

**返回：** `serial.Serial` - 已连接的 pyserial 对象

<Note>
对于不在上表中的 `parity` 值，`connect()` 会抛出 `ValueError`。
</Note>
