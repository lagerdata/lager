...
- 您的持有者字符串，或它的作用域。
- 您的普通用户名。例如，CI 作业的运行器账户所做的 `lager boxes lock` 预留。
- 您的用户名，前提是持有者的形式为 `<origin>:<id>:<name>:<email>`，并且 `<email>` 就是该用户名。比较时不区分大小写。其他工具（例如控制平面面板）可能会写入这种形式的持有者。

Box 比较的是完整字符串。因此 `lager boxes unlock` 会先读取锁。如果该锁属于您，解锁命令会发送 Box 所存储的持有者字符串。
...
| `LAGER_LOCK_WAIT=<seconds>`   | 覆盖冲突时的等待时间。`0` = 快速失败（开发机默认），较大的值 = 耐心排队（CI 默认）。不是整数的值会打印一条警告，并采用当前环境的默认值。 |
| `LAGER_CI_OVERRIDE=<any>`     | 在 CI 中使用用户计算机的锁行为：持有者是您的普通用户名，默认等待时间为 `0`。若只想改变 `lager exec` 的行为，请改用 `LAGER_EXEC_IN_PLACE`，它不影响锁。 |
...
Box 按完整字符串精确比较持有者名称。CLI 使用[命令如何识别自己的锁](#命令如何识别自己的锁)中的规则。其他工具（例如控制平面面板）可能会锁定这台 Box，并把持有者写成 `<origin>:<id>:<name>:<email>`。`lager boxes` 会把这样的持有者显示为 `<name>`。要让这样的锁属于您，请用 `lager defaults add --user` 或 `LAGER_USER` 把您的用户名设置为该邮箱。
...
不加 `--force` 时，解锁命令会先读取锁。如果按上文的规则该锁属于您，解锁命令会发送 Box 所存储的持有者字符串。
