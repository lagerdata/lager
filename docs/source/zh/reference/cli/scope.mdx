...
该条件目前还不选择极性：每个 `positive` 值与对应的 `negative` 值设置的是同一个宽度条件。缺少所需的宽度时，命令不会更改任何宽度条件，并打印一条提示。
...
| `--data-width INT` | 数据宽度，单位字节：`1` 至 `5` | `1` |
| `--direction` | 方向：`read`、`write`、`read_write`（两者皆可） | `read_write` |
...
| `--data HEX` | 要匹配的数据值（十六进制），最大为 2^width − 1 | - |
| `--data-width INT` | 数据宽度，单位位：`4` 至 `32` | `8` |
| `--clk-slope` | 时钟边沿：`rising`、`falling` | `rising` |
...
| `--trigger-on` | 条件：`start`、`data`、`error`（帧错误）、`cerror`（奇偶校验错误） | `start` |
| `--data HEX` | 要匹配的数据模式（十六进制） | - |
