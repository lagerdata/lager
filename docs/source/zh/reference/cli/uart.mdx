...
两个 Net 可能指向同一台设备。这种情况下，错误信息会指出设备路径以及持有它的 Net：`UART device /dev/ttyUSB0 is already in use by net 'SERIAL2'`。接管命令中给出的也是那个 Net。
...
`--force` 会释放持有该 Net 的每一个会话，然后再连接。如果它确实释放了某个会话，会打印 `Released the session that held 'SERIAL1'`；如果没有任何会话持有该 Net，则什么也不打印。被您顶替的那个会话会停止接收数据，并打印
`Error: UART net 'SERIAL1' was taken over by another client`。

`--force` 只作用于您指名的那个 Net。如果错误信息指出的是设备路径，说明同一设备上的另一个 Net 持有它。请接管错误信息中指出的那个 Net。较旧的 Box 不会指出它，此时请用 `--sessions` 找到那个 Net。
...
- `This box does not report UART sessions; update it with lager update.`
  ...
  `--force` 会打印一条警告，说明该 Box 版本太旧，不支持它。
...
<Note>
与每一条会建立连接的 `lager uart` 命令一样，`--force` 会获取 Box 锁。如果另一位用户持有 Box 锁，它会失败并报出
`Error: Box 'my-lager-box' is locked by <holder>`。`--sessions` 不获取 Box 锁，因此在另一位用户持有 Box 锁时也可以使用。
</Note>
