...
所有 Box 会同时查询。已关机或正在更新的 Box 只消耗它自己的超时时间，不会再让其他 Box 一起等待。在终端中，表格会立即打印出来，每一行先显示 `pending`。各行会随着对应 Box 的应答逐一填入。下方的倒计时显示尚未应答的 Box 还剩多少时间：
...
| `pending` | 灰色 | 仅在终端中显示，表示该 Box 尚未应答 |
