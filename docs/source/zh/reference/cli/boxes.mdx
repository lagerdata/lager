...
所有 Box 会同时查询。已关机或正在更新的 Box 只消耗它自己的超时时间，不会再让其他 Box 一起等待。在终端中，表格会立即打印出来，每一行先显示 `pending`。尚未应答的 `status` 和 `locked by` 单元格旁边会有一个旋转指示符。各行会随着对应 Box 的应答逐一填入。下方的倒计时显示尚未应答的 Box 还剩多少时间：
...
实时表格始终显示 `locked by` 列，因为在 Box 应答之前无法知道锁的状态。稍后才出现的列会让下方的每一行发生位移。

按 `Ctrl+C` 可以停止等待，同时保留已经应答的 Box：其余的会标记为 `cancelled`，表格随即打印。始终不应答的 Box 会在倒计时归零后不久被放弃，并标记为
`no response`，因此该命令一定会结束。

当输出被重定向到文件或 CI 日志时，该命令不会使用任何实时显示。它只打印一次最终表格，与上面所示完全相同。
...
在这张最终表格中，只有当某台 Box 被锁定时，`locked by` 列才会出现。来自分支或提交的版本会在括号中显示 ref，例如 `0.47.0 (main)`。使用发布标签的 Box 只显示版本号。
...
| `no access`、`auth server down`、`token rejected` | 红色 | 该 Box 受访问网关保护，且网关拒绝了请求。需要访问授权或管理员处理，而不是重新登录 |
| `pending` | 灰色 | 仅在终端中显示，带旋转指示符，表示该 Box 尚未应答 |
| `no response` | 红色 | 该 Box 始终没有应答，在倒计时结束后被放弃 |
| `cancelled` | 红色 | `Ctrl+C` 停止等待时，该 Box 还没有应答 |
...
**选项：**
- `--box`（必填）- 要解锁的 Box 名称
- `--user` - 解锁时使用的持有者，用于以您的用户名之外的名称记录的锁
- `--force` - 即使是别人加的锁也强制解锁

不加 `--force` 时，解锁命令会释放 CLI 认为属于您的锁，例如您自己的 CI 作业的自动锁。
