...
| `--external-id TEXT` | 该文档在外部文档存储库中的 ID。 |
| `--external-url TEXT` | 该文档在外部文档存储库中的 URL。 |
| `--pages TEXT` | 可选的页码/图纸提示，例如 `"3-5"` 或 `"POWER sheet"`。 |
| `--notes TEXT` | 可选的自由格式备注。 |

`--url`、`--repo-path`、`--external-id` 和 `--external-url` 中至少要提供一个。该引用会追加到与 `--kind` 匹配的列表中（`schematic` → `schematic_refs`，`datasheet` → `datasheet_refs`，
...
带有 DUT 编辑器的控制平面通过 Box 端口 9000 上的 `PUT /dut` 写入同一个块，`/status` 通过 `capabilities.dutSync` 声明它。[实验台清单](/source/zh/reference/cli/bench)报告 `dut_updated_at`，即写入方记录的时钟和文件 mtime 中较晚的那个，因此双方都能判断哪个副本更新。`lager dut edit` 和 `add-doc` 也记录这个时钟。
