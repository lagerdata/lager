...
<Note>
**默认情况下，工具接口是只读的。** 上面这七个工具不驱动硬件（设置电压、翻转 GPIO、烧录固件），也不改动 Box。这些都发生在智能体编写、并用 `lager python` 运行的测试脚本里，或者发生在专门的 [CLI 命令](/source/reference/cli)中。
...
<Steps>
  ...
  <Step title="编写并运行">
    ...
    本地 Box 名称只是任意的客户端侧别名。`--box` 接受原始 IP，因此不需要任何注册：运行 `lager python path/to/test.py --box <box-ip>`。
    runnable 也可以是一个**文件夹**（入口为 `main.py`），它会同步并导入其中的全部内容 —— 这在分发可复用的辅助模块时很方便：
