...
**`timeout-minutes` 是作业占用实验台的最长时间。** 异常终止的作业会一直占用实验台和它的锁，直到 GitHub 停止该作业。请设置一个您能接受的时间，不要使用默认的 360 分钟。
...
请把下面的脚本放在 `tools/retry-hil.sh`。它只在装置故障之后重试。它在两次尝试之间对探针和被测设备断电再上电，也会把卡住不结束的测试转换成可以重试的失败。
...
脚本中有两处很重要。`timeout` 命令把卡住不结束的测试转换成脚本可以重试的失败。文字模式把退出码 1 转换成装置故障。这是必要的，因为 Box 不可用时给出的恰好是设备故障的退出码。
