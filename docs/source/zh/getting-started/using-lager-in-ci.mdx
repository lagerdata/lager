...
Box 只为完全相同的持有者字符串释放锁。因此 `lager boxes unlock` 会先读取锁。该锁可能是同一作业的自动锁，也可能是运行器账户的一次 `lager boxes lock` 预约。对于这些锁，解锁命令会发送 Box 所存储的持有者字符串。因此这一步可以释放这两种锁：
...
运行器账户的 `lager boxes lock` 预约不会阻塞该账户的 CI 命令。CI 命令会使用该预约，并且不会释放它。

通用 CI 提供方会让一台主机上的所有作业使用同一个作用域。对于这些作业，解锁命令只释放持有者字符串完全相同的锁。
...
| 变量 | 作用 |
|---|---|
| `LAGER_LOCK_WAIT` | 冲突之后等待的秒数。CI 默认 1800，用户默认 0。不是整数的值会给出一条警告，并采用默认值。负值按 0 处理。 |
...
- 在运行于容器中的 CI 作业中，它直接运行命令。作业已经在该镜像中，所以没有容器需要启动。作业容器中通常也没有 Docker。

CLI 会先读取 CI 变量，再检查当前进程是否处于容器中。两者都成立时才算数。请用 `container:` 把镜像交给作业：
...
GitHub Actions、GitLab CI、Drone 和 Bitbucket Pipelines 上的作业会得到第二种行为，但只有当它运行在容器中时才会如此。没有 `container:` 的作业运行在运行器本身，此时 CLI 会在那里启动容器，并打印一行说明。
...
若要自行选择运行方式，请设置 `LAGER_EXEC_IN_PLACE`。设为 `1` 表示就地运行命令，设为 `0` 表示启动容器。要让 `0` 生效，作业必须有 Docker：
...
`LAGER_EXEC_IN_PLACE` 只影响 `lager exec`，不影响其他任何命令。设置它之后，Box 加锁行为保持不变。

<Warning>
`LAGER_CI_OVERRIDE` 同样可以启动容器，但代价更大。它会让看到它的每条 `lager` 命令都停止 CI 识别。此时锁冲突会立即报错，并且锁持有者是用户名。请优先使用 `LAGER_EXEC_IN_PLACE=0`，并且不要为整个作业设置 `LAGER_CI_OVERRIDE`。
</Warning>
...
退出码 1 只有一个含义：Box 需要更新。下列失败都会让 Box 的状态无法确定，因此它们都给出退出码 2：
...
- Box 被其他持有者锁定。
- CLI 没有确认该 Box 的 SSH 密钥，或者 Box 上的 `git rev-list` 失败。

当确实需要更新时，输出以 `Run without --check to apply.` 结尾。示例脚本就依据这段文字判断。退出码 2 不提供关于 Box 的任何信息。

<Note>
0.49.0 之前的 CLI 版本对上述失败也给出退出码 1。示例脚本正是因此才匹配 `Run without --check to apply.` 这段文字。它在新旧两种退出码下都是正确的。
</Note>
...
| `LAGER_LOCK_HOLDER` | 为锁持有者指定不同的身份。 |
| `LAGER_LOCK_WAIT` | 锁冲突之后等待的秒数。负值按 0 处理。不是整数的值会给出一条警告，并采用默认值。 |
| `LAGER_LOCK_TTL` | 锁的最长存活时间。用 `none` 表示不限时。 |
| `LAGER_LOCK_HEARTBEAT` | 两次心跳之间的秒数，默认 60。 |
| `LAGER_AUTO_LOCK_DISABLE` | 任何非空值都会关闭自动锁，`0` 也会关闭。 |
...
| `LAGER_EXEC_IN_PLACE` | `1` 让 `lager exec` 就地运行命令，`0` 启动容器。它不影响其他命令，也不影响锁。 |
...
| | 130 | 有人或某个信号停止了该命令。 |
| | 128+N | 信号 N 停止了 Box 上的脚本。 |
| `lager update --check` | 0 / 1 / 2 | 正确 / 需要更新 / CLI 未能确定 Box 的状态。 |
...
要查找持有者，请用 `lager boxes`。以 `ci:github:` 开头的持有者会显示仓库、运行、作业和运行器。请等待，或与持有者沟通。不要在作业中使用 `unlock --force`。
...
作业在容器中，但 CLI 试图再启动一个容器。请确认没有设置 `LAGER_CI_OVERRIDE`，并且 `LAGER_EXEC_IN_PLACE` 不是 `0`。如果该容器没有留下常见的标记，请设置 `LAGER_EXEC_IN_PLACE=1`。低于 0.46.2 的 CLI 总是会启动容器，请升级 CLI。请参阅 [第 6 节](#用-lager-exec-构建)。

**`lager exec` 现在会启动容器，而以前是就地运行命令。**
该作业没有声明 `container:`，因此命令一直运行在运行器上，而不是在镜像中。CLI 现在会说明这一点并改用 Docker。请给作业加上 `container:`，或者设置 `LAGER_EXEC_IN_PLACE=1` 以保持原有行为。
