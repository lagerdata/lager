...
[^plugable]: Unlike the Acroname and Yepkit hubs, Plugable ships no SDK. Control
    ...
    depends on the individual hub genuinely implementing it — a hub that
    advertises PPPS but does not switch VBUS is detected and reported rather
    than silently ignored. Matched by VID:PID `2230:5411`, which Plugable reuses
    across its RTS5411 dock line; validated on a UD-CAM. The dock cascades two
    internal hubs and only the four external USB-A sockets are exposed. Requires
    the udev rule for vendor `2230` (shipped in `99-instrument.rules`).
