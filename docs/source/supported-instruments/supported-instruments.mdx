...
Lager exposes only the user tier, so the four lager ports are the four external
Type-A sockets (on the validated model: three on the rear, one on the front) and
the dock's own network adapter can never be switched off by accident.

**Which lager port number is which physical socket is not defined** — nothing
ties USB hub port order to the order of connectors on a case, and it may differ
between models. Determine it once, empirically, by moving a device that
enumerates (any USB stick, or a debug probe) through each socket in turn and
reading back which port it lands on:

```bash
lager ssh --box <BOX> -- ls -l /sys/bus/usb/devices/<hub-path>
```

A **charge-only cable cannot be used for this** — it presents no device to the
hub, so it is indistinguishable from an empty socket (see the warning above).

<Warning>
**Moving a cable between sockets silently repoints the net.** A `usb` net names a
hub *port*, not the device plugged into it, so after someone reseats a cable the
same command switches something else — with no error, because from the hub's
point of view nothing is wrong.

This cuts the opposite way to every other instrument on the box. A debug probe,
a USB-UART bridge, and a power supply are addressed by **serial number**, so
their nets follow the device wherever it is plugged in. A hub port net is
...
the hub and drive whichever port it is actually on, rather than hard-coding a
net name.
</Warning>

Verified by writing a volatile setting to a device on a user port, cycling power, and
reading the power-on default back — a real cold boot, not a logical disconnect.
The upstream tier's behaviour is inferred from re-power timing (a device there
reasserts CONNECT within a millisecond, so it never lost power) rather than from
a register, so treat it as strong evidence rather than proof.
...
implements that pairing, but no SuperSpeed-linked dock has been available to
validate it; a topology it cannot pair unambiguously is refused rather than
