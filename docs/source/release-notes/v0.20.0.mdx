...
- **Version-skew warning** prints once per CLI session to stderr when the CLI's minor version is ahead of the box's by one or more. Without it, a 0.19 CLI talking to a 0.18 box fails with an opaque first error. Cached per-process by box IP; fails open on any error so a flaky network can never break a working command.
...
- **Documented "TUIs are laptop-only"** in `box/lager/README.md`. A TUI run directly on the box is a second `pyvisa-py` client competing with hardware-service for interface 0. The OS-level `device_lock` makes this case detect-and-fail-clean instead of silent EBUSY, but the right answer is still to launch TUIs from the laptop CLI.
