...
- **Tune or disable the auto-resume.** Set a longer (or shorter) wait with `pause("...", timeout=1800)` or `lager python ... --breakpoint-timeout 1800`; use `timeout=0` (or `--breakpoint-timeout 0`) to wait indefinitely; set `LAGER_BREAKPOINTS=off` to turn every breakpoint into a no-op for a clean run.
