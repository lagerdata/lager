...
Provision an ESP32 onto a WiFi network over Bluetooth, using Espressif's BluFi
protocol — the flow a phone app performs during onboarding, driven from a test.
...
- **`sta_conn == 0` means connected.** Zero is success here, not failure, and the
  ...
  3 has associated but has no DHCP lease, which is a different bug from a wrong
  password.
- Every action except `scan` connects over BLE and negotiates BluFi security first,
  so the budgets are wide: the box-side connect timeout is 20 seconds, the client
  allows that plus 40, `provision` gets 20 plus 60 because end-to-end provisioning
  routinely takes over 30 seconds, and `wifi_scan` gets 20 plus 30 because the target
  runs its own scan.
