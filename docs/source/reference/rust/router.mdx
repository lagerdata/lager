...
DHCP control, security profiles and access lists. None of these have a typed wrapper
in this crate yet.
...
- **This net sends no role hint.** A router net can be saved with role `router` or
  the legacy `mikrotik`. The box verifies a supplied hint exactly, so a hint will fail
  against whichever spelling the net does not use. A consequence you will see: a
  missing router net reports `Net 'router1' not found` without naming a role, where
  other net types
