...
| LabJack U3 | DAC0, DAC1 | Last value that the box set; output range 0.04 V to 4.95 V |
| MCC USB-202 | DAC0, DAC1 | Not supported |

## Notes

- Leave the output where the next test expects it. A DAC holds its last value after
  your test ends; setting it back to `0.0` in teardown is usually right. A LabJack U3
