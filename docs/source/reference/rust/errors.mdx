...
The crate did not reach the box at all. This variant also covers a failed Socket.IO
connect.
...
The crate did not parse the response into the expected shape. The cause is a truncated
body, an unexpected field type, or a missing required field.
