...
The crate is designed so that your HIL suite is just ordinary Rust
integration tests. These are files under `tests/`, run by `cargo test`, that
...
Timeout budgets mirror the Lager CLI. Quick commands use 10 s. Some
operations block on the box for a caller-controlled duration: the watt and
energy integration windows, and `wait_for_level`. Those operations widen or
...
This is the convention that the crate itself uses. Its own suite is
hermetic, and `cargo test` runs against a mock box. Its hardware smoke tests
