...
<Warning>
  **A capability flag mirrors route registration, not the box's ability to do the
  ...
  installed fails the same way. Use these flags to decide whether an endpoint exists, not
...
<Note>
  `lock_guard()` exists only on the blocking client. The async client has every other
  lock method, but no RAII guard.
</Note>
...
<Warning>
  **Ceilings cap setpoints, not protection trips.** With a 3.6 V ceiling,
  ...
  refused `set_ocp` on such a net comes from the instrument's own hardware limit,
  not from the ceiling. Do not rely on a safety limit to bound an OVP or OCP
  setting.
</Warning>

There is deliberately no `max_power`. One setter call establishes either a voltage or
a current, never both, so the box cannot evaluate a power ceiling honestly. It
therefore refuses the key outright.
