...
<Note>
The DUT context is also part of the
[bench manifest](/source/reference/mcp/overview#the-bench-manifest), and a
control plane reads and writes it over `GET|PUT /dut` on the box's port 9000.
</Note>
...
Each net carries a single-sentence **purpose** plus optional **notes**. Two
more fields serve the test author. **dut_connection** says where the net lands
on the DUT: a connector, pin or test point. **test_hints** holds one-line
advice, one entry per hint. Set purpose, notes and tags in the Net Manager TUI:
...
`--dut-connection` and `--test-hint` are the only way to set those two fields
from the CLI; the TUI edits purpose, notes and tags.
</Note>

<Warning>
`/etc/lager/bench.json` can hold a `net_overrides` entry for a net. If that entry sets
`purpose`, `notes`, `tags`, `dut_connection` or `test_hints`, agents see the override
value, not the value saved on the net. The TUI and `lager nets describe` give no
warning about this. `discover_bench(net_name)` and the [bench
manifest](/source/reference/mcp/overview#the-bench-manifest) carry
`metadata_sources`. It names the file each field came from, `bench.json` or
`saved_net`, so you can see when an override shadows a saved value. Remove the
field from the override before you set it on the net.
</Warning>
...
| `external_id` | ID of the document in an external document store. |
| `external_url` | URL of the document in an external document store. The agent opens it with its own credentials. |
| `pages` | Optional page/sheet hint, e.g. `"3-5"` or `"POWER sheet"`. |
| `notes` | Optional free-form note. |

You must supply at least one of `--url`, `--repo-path`, `--external-id` or
...
| Doc in a document store that your agent can reach | `--external-id` or `--external-url` | The agent opens it through its own connector, with its own credentials. The box never authenticates. |
...
The agent looks for a `repo_path` in your project root first. If the file is not
