...
- **Tags** (optional) — for filtering, e.g. `flash`, `boot-critical`.

<Note>
This replaces the older four-field form (`description`, `dut_connection`,
`test_hints`, `tags`). Existing nets are migrated automatically: their old text
is folded into `purpose` and `notes` on load, so nothing is lost and no manual
migration is needed.
</Note>
...
[`lager box dut`](/source/reference/cli/box-dut) command group.
...
The MCP server reads the bench config when it starts. After editing DUT context
or attaching a doc, it needs to re-read `/etc/lager/bench.json` before agents see
the change. Either:

- have a connected agent call the `box_manage` tool with `action="reload"`, which
  re-reads the config from disk and rebuilds the capability graph, **or**
- restart the box service so it reloads on startup.

Agents then see the new context on their next `discover_dut()` or
`lager://dut/overview.md` read.
