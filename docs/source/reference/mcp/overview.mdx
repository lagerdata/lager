...
that; see [Optional tool gates](#optional-tool-gates).) To **execute** a test,
the agent writes a Python file locally and
...
the container, but it is reachable only on the internal `lagernet` Docker network,
