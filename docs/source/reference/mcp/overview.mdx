...
- `reference_entries`, those entries themselves: methods, gotchas and an example
  snippet for each net type on the bench.

It is built from the same loaded state the MCP tools read, so the two never
disagree. `bench.dut_updated_at` says when the DUT context last changed, so a
client that keeps its own copy can tell which side is newer.
