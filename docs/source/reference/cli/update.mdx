...
Release tags publish a box container image to the GitHub Container Registry. When
a pull is enabled, the client resolves the tag to an immutable digest, pulls that
digest pinned to the box's own architecture, and verifies the image reports the
version that was asked for before using it. Any miss at any step — a branch target
rather than a release tag, an unpublished tag, an unreachable registry, a
mismatched image — falls back to the local build that has always run.

<Note>
For `lager update`, pulling is **off by default**. It changes how the most
load-bearing command in the fleet obtains the code it runs, so it is soaking on
...
way on both, and is the switch that routes a fleet around a bad published image
without waiting for a CLI release.
</Note>
...
(only release tags publish one), so a SHA builds on the box just as a branch
does, and the commit must be reachable from a branch or tag on the remote.
...
- `--check` reports what would change without modifying the box, so it is the safe
