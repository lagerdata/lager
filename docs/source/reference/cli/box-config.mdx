...
- **The host firewall starts governing the box's ports.** On `lagernet` the
  ...
  interfaces are `lo`, `docker0`, `tailscale0` when present, and one you name
  with `--corporate-vpn`. It denies those ports everywhere else, and it does
  not allow them on a plain LAN interface. So a box you reach over a route that
  script did not allow becomes unreachable the moment the ports stop being
  published. `lager install` runs that script by default, so most boxes carry
  these rules.
...
`sudo -n /usr/sbin/ufw status`, and Lager does not grant that command. If `sudo`
asks for a password on the box, `apply` refuses and prints the one sudoers line
that the check needs. Add that line to a sudoers file of your own with
`sudo visudo -f`. Lager changes only the sudoers files that it owns, so your
grant stays in place after `lager update`.
...
Leave this at the default unless you need the adapter inside the container. A
box whose lager predates the setting says so.

**Exit codes for `apply`:** `0` applied, `3` the container is up on the
previous config, `1` anything else.
