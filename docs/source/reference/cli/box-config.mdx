...
It then checks that the firewall admits the control-plane ports there. When
they are not admitted, it prints the exact per-interface `ufw allow` command.
It changes nothing when it refuses. `--skip-host-network-check` overrides it; use that only with another
way into the box.
