...
| `mcp-token` | The optional bearer token on the box MCP server. Not part of the config |
...
token instead, see [`mcp-token`](#mcp-token).
...
```bash
lager box-config env set LAGER_MCP_NO_PUBLISH=1 --box my-lager-box
lager box-config apply --box my-lager-box
```
...
The optional bearer token on the box [MCP server](/source/reference/mcp/overview)
(port 8100). The token is off by default, and the server then asks for no
credential.

```bash
lager box-config mcp-token status [--json] --box my-lager-box
lager box-config mcp-token enable --box my-lager-box
lager box-config mcp-token rotate [--yes] --box my-lager-box
lager box-config mcp-token disable [--yes] --box my-lager-box
```
...
| State | Meaning |
|-------|---------|
| `enabled` | The server requires the token |
| `disabled` | No token exists. The server asks for no credential |
| `unreadable` | The token file exists, but the server cannot read it. The server refuses every request |
| `empty` | The token file has no content. The server refuses every request |

In the `unreadable` and `empty` states the server answers `503`, not `401`. A
...
crosses the network in clear text, and so does the reply to `enable`. Keep the box
