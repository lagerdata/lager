...
and every other port stays published. The MCP server performs no authentication,
so use this variable to take it off the network. Clients on the `lagernet` Docker
network still reach it.
