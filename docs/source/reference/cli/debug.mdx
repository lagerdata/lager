...
| `erase` | Erase flash memory (all of it or a given range; 1 MiB on DA1469x by default) |
...
The command prints the exact address to use. On a plain Lager Box, it prints the address of the box and returns.

**Boxes behind a gateway:**

Some boxes are behind an authenticating gateway (see [`lager login`](/source/reference/cli/login)). The gateway checks your sign-in on each connection. The GDB port of such a box is not open to the network, so GDB cannot connect to the box directly.

On such a box, `gdbserver` starts the GDB server and then opens a tunnel through the gateway. The tunnel listens on `localhost` only, on the same port number as the GDB server on the box. The command stays in the foreground while the tunnel is open:

```text
JLinkGDBServer started!
GDB server running on my-lager-box. Connect to localhost:2331
Connect with: arm-none-eabi-gdb -ex 'target remote localhost:2331'
Tunnelling through the box's gateway. Press Ctrl-C to close the tunnel.
```

```bash
# In a second terminal
arm-none-eabi-gdb firmware.elf -ex 'target remote localhost:2331'
```
...
If the command says that the gateway of the box does not support debug tunnels yet, the gateway must be updated. Speak to the administrator of the box.
...
`--erase-start` and `--erase-size` set the range of that erase, on both backends. Without them, `flash` erases the whole chip, or 1 MiB from `0x16000000` on a DA1469x. The `erase` section below describes the range, and the box version it needs.
...
# Erase 2 MiB from the start of QSPI on a DA1469x, then flash
lager debug flash --bin build/app.bin,0x16000000 --erase-start 0x16000000 --erase-size 2M --box my-lager-box
...
Erase flash memory on the target: all of it, or the range you give with `--erase-start` and `--erase-size`. **This is a destructive operation.**
...
lager debug erase --box my-lager-box --yes --erase-start 0x16000000 --erase-size 2M
...
The output includes a `Features` line: the capabilities the box's debug service supports beyond its original requests, such as `erase_range` for `--erase-start` and `--erase-size`. A box that predates the list prints `none reported`.
