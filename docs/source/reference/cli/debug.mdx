...
`Target attached` reads `Unknown` when the box cannot establish an answer.
Two cases give that result: a box older than this field, or a probe that
cannot run because a debugger already holds the session. `Unknown` does not
mean the target is absent.
