...
about the part: the box reads the target to answer it. They are reported
separately because they genuinely differ -- a gdbserver can outlive the device
it was talking to, and a command that flashes or erases cares about the second,
not the first.

`Target attached` reads `Unknown` when the box could not establish an answer:
a box older than this field, or a probe that could not run because a debugger
is already attached to the session. `Unknown` does not mean the target is
missing.
