...
| `model-create` | Create a custom battery model in a memory slot from a CSV file |
| `model-export` | Export a saved battery model's curve to a CSV file |
...
**Arguments:**
- `NETNAME` - Name of the battery Net
- `VALUE` - Maximum current limit (amps), omit to read. Must be between 0.001 and 6.0 A (Keithley 2281S limit); values outside this range are rejected.
...
<Warning>
`model-create` overwrites the slot's previous model. The instrument has no way
to delete a model from a slot. Once a slot holds a model, you can only
...
accepts. You can therefore export a saved model, edit it, and write it back to
a slot (`model-export` → edit → `model-create`). Read-only: exporting does
not recall or change the active model. Exporting an empty slot is an error —
