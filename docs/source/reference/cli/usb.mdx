...
```bash
# The device did not return within 5 seconds (exit code 1)
# Error: USB port 'USB1' power-cycled, but the device did not come back before the timeout
...
`cycle` exits `1` when the device did not come back. The other three results exit `0`.
An empty port and an unreadable topology confirm nothing about a device. A
script that needs the device must check the output text for those two results.
A box too old to report whether the device came back makes `cycle` exit `0` for
all four results.
