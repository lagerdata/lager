...
## The scope and its channels

An oscilloscope is two kinds of net. A **`scope`** net is the instrument
itself; a **`scope-channel`** net is one of its inputs. Plugging in a
two-channel PicoScope offers three nets in `lager nets tui`: the two channels
and the scope they belong to.

The split follows the hardware. Six settings belong to a channel, and the rest
belong to the instrument:

| Belongs to a `scope-channel` net | Belongs to the `scope` net |
|---|---|
| `enable`, `disable` | `start`, `stop`, `force` |
| `scale` (volts/div) | `timebase`, horizontal position |
| `coupling` | `trigger` (source, slope, level, mode) |
| `probe` | `cursor` |
| vertical offset | `autoscale`, reported capabilities |
| `measure` | |

Both are addressed the same way, so nothing about the command changes:

```bash
lager scope vbus coupling ac      # a channel net
lager scope picoscope1 timebase 1ms   # the scope net
```

<Note>
A command that belongs to the instrument is also accepted on one of its
channel nets, because a channel names exactly one scope and there is nothing
to be ambiguous about. Scripts written before scope nets existed keep working
unchanged.

The reverse is refused. `lager scope picoscope1 coupling ac` names no channel,
and rather than guess one the CLI names the channel nets that would have
worked.
</Note>

<Note>
Boxes set up before scope nets existed had only channels, saved with the role
`scope`. Those are converted the first time the box reads its nets: each
becomes a `scope-channel` net keeping its name and pin, and a `scope` net named
after the model — `picoscope1` — appears alongside them. Nothing needs to be
re-added, and the new net can be renamed.
</Note>
...
| `coupling` | Set channel coupling mode (dc, ac; gnd is Rigol only) | Both |
| `probe` | Set probe attenuation ratio | Both |
| `scale` | Set vertical scale (volts per division) | Both |
| `timebase` | Set horizontal timebase (seconds per division) | Both |
| `measure` | Measure waveform characteristics | Both (see note) |
| `trigger` | Configure trigger settings | Both (see note) |
| `cursor` | Place cursors over the capture and read the deltas | Both; `set-a`/`set-b`/`move-a`/`move-b`/`hide` are Rigol only |
...
<Note>
Measurements work on both instruments, but they are computed differently. A Rigol
answers a SCPI query about what it already displays; for a PicoScope the daemon
computes the value from a captured block, because no PicoTech API has an
equivalent of `:MEAS:VPP?`. The consequence is that `--display` and `--cursor`
are Rigol only: they put the reading on the instrument's own screen, and a
PicoScope does not have one.

While a PicoScope is acquiring, measurements and cursor readings are computed
from the capture most recently streamed out, so they describe the trace the web
UI is drawing rather than a separate capture taken a moment later. Stopped, they
describe the last capture, as the readouts on a stopped bench scope do.
</Note>
...
| PicoScope | 2000, 2000a, 3000a, 4000a, 5000a series | Real-time streaming, web-based visualization, CSV export, edge trigger, daemon-computed measurements |

The channel count, voltage ranges, bandwidth, memory depth and ADC resolution of
an attached PicoScope are detected at runtime rather than assumed from the
series, so a 2-channel 2204A and an 8-channel 4824A both work without
configuration. `lager scope <net> capabilities` reports what was detected.

### Feature Comparison

| Feature | Rigol MSO5000 | PicoScope |
|---------|---------------|-----------|
| Channels | 4 analog + 16 digital | 2-8 analog, detected at runtime |
| Real-time streaming | No (single capture) | Yes (continuous) |
| Measurements | 11 built-in types | 14 types, computed from the capture |
| Trigger types | Edge, pulse, I2C, SPI, UART | Edge only |
| Cursor control | XY cursors on the instrument display | Time/voltage cursors over the captured samples |
| Autoscale | Yes | No |
| Web visualization | No | Yes (HTML5) |
| Data export | Via measurements | CSV streaming |
| Control method | SCPI over USB/LAN | Vendor SDK, via the box daemon |
...
| `gnd` | Ground reference (0V baseline). Rigol only |

`gnd` is refused on a PicoScope 2000-series unit, which has no ground switch --
`ps2000_set_channel` carries a single flag for coupling. Use `ac` to block the
DC component, or unplug the probe to see where zero sits.
...
The range above is what the command accepts, not what a given unit can do. A
scope samples at a fixed ladder of intervals, so the request is rounded to the
nearest, and how fast a screen it can reach at all depends on the block depth
as much as the sample rate: a 2204A at 100 MS/s over 8000 samples bottoms out
at 8 us/div. Both show up in the readback, which reports the scale in use
...
Measure waveform characteristics on either instrument. A Rigol is asked for a
reading it already computes; for a PicoScope the box captures a block and
computes the value from the samples, so no streaming or CSV export is needed.

Because one PicoScope capture yields every quantity at once, reading several
values costs several captures. A script that wants the whole set should use
`measure_all` from the [Python API](/source/reference/python/scope) instead of
...
|--------|-------------|---------|
| `--mode` | Trigger mode: `normal`, `auto`, `single` | unchanged |
| `--coupling` | Trigger-path coupling filter: `dc`, `ac`, `low_freq_rej`, `high_freq_rej` (Rigol only) | unchanged |
| `--source NET` | Trigger source net | Current net |
| `--level VOLTS` | Trigger level in volts | - |

<Note>
`--coupling` here is the filter on the way to the trigger comparator, not the
channel's input coupling. They share a word and do different things: this one
lets a drifting or noisy edge be triggered on without altering the displayed
trace, while `lager scope <net> coupling` changes what the channel passes and
does move the trace. Only a Rigol has the trigger filter; on a PicoScope the
comparator sees the channel as it is, and the flag is refused rather than
quietly applied to the input.

Every option leaves what it does not name alone. `--mode` and `--coupling`
used to default to `normal` and `dc`, so adjusting a level re-applied both --
a trigger armed for `single` went back to `normal` without being asked.
</Note>

<Note>
`--mode` says how a running sweep triggers, and `auto` is the one that will
show you something regardless: it triggers itself if nothing crosses the level,
so the trace keeps moving whether or not the trigger is set usefully. `normal`
updates the screen only on a real trigger, which is what holds a repetitive
waveform still in one place.

`--mode single` also arms a single-shot, the same as `lager scope <net>
single`, because a mode that captures once is not a state a scope can sit in
waiting: `run` promotes it to `auto`, single-shot being the one mode that
cannot run continuously.

Starting a capture no longer forces `auto`. It used to, so selecting `normal`
and then running left the scope free-running with the trigger ignored, and
nothing read the mode back to show it.
</Note>
...
# Either edge, with the trigger path AC coupled (Rigol only)
...
`cursor time`, `cursor volts` and `cursor off` place cursors over the
**captured samples** and work on any scope. They are typed rather than
dragged: there is no knob for them and no handle on the plot to grab. The box
holds the pair, so a cursor placed from the terminal is the one the web UI
draws, and the same `cursor` verb works in that UI's console.

`cursor set-a`, `set-b`, `move-a`, `move-b` and `hide` position markers on a
**Rigol's own front display** in screen coordinates, over SCPI. A PicoScope
has no display to put them on.

### `cursor`

With no subcommand, reads whatever cursors are set.

```bash
lager scope NET_NAME cursor [--box BOX] [--mcu MCU]
```

### `cursor time`

Place the two time cursors, in seconds relative to the trigger. Negative is
before the trigger; half of a capture sits there.

```bash
lager scope NET_NAME cursor time T1 T2 [--box BOX] [--mcu MCU]
```

Reports the interval between them, its reciprocal as a frequency, and the
voltage of the trace under each — so a pair placed a cycle apart reads the
frequency off directly. The voltages are interpolated between samples, since
a cursor lands where it was typed rather than on the sample grid.
...
```bash
lager scope NET_NAME cursor volts V1 V2 [--box BOX] [--mcu MCU]
```

Reports the difference. No capture is needed for this pair, so it answers
...
```bash
lager scope NET_NAME cursor off [--box BOX] [--mcu MCU]
```

**Examples:**

```bash
# Measure a period: put a pair one cycle apart and read 1/dt
lager scope ANALOG1 cursor time 0 0.001 --box my-lager-box

# Read the two pairs back against the live signal
lager scope ANALOG1 cursor --box my-lager-box

# Mark a voltage window
lager scope ANALOG1 cursor volts 3.0 3.6 --box my-lager-box

lager scope ANALOG1 cursor off --box my-lager-box
```

<Note>
The pairs are independent: placing time cursors leaves voltage cursors where
they were. Each pair is placed as a pair, because one cursor on its own reads
nothing — the quantity wanted is the difference between two.

A cursor outside the captured window reports no voltage rather than the
nearest sample, which would be a reading from a different moment. The reply
names the window so it is clear why.
</Note>
...
Rigol only. Move a display cursor by a relative offset from its current
position.
...
Rigol only. Hide the instrument's display cursors.
...
Everything reaches the scope through the box HTTP server on **port 9000**, the
same port every other net uses. The daemon that owns the PicoScope binds
loopback only and is not reachable from outside the box:

| Endpoint | Purpose |
|----------|---------|
| `:9000` `GET /scope` | The web visualization UI |
| `:9000` `POST /net/command` | Control: enable, scale, timebase, trigger, measure |
| `:9000` `GET /scope/<net>/stream` | A ticket: a token, the data-socket path, and the detected capabilities |
| `:9000` `GET /scope/<net>/ws` | WebSocket carrying capture frames, using the token above |
| `:9000` `/scope` (Socket.IO) | Live acquisition state for the UI |
| `127.0.0.1:8085` and `/tmp/lager-scope.sock` | The daemon itself. Loopback only; the port 9000 relay is the only way in |

Capture frames cross the relay in **LSCP/1**, a packed binary format: a 64-byte
header followed by contiguous `int16` ADC counts. The relay forwards frames
without decoding them, so its cost does not grow with capture size.

<Note>
Earlier versions published the daemon directly on ports 8081-8085 and streamed
to the browser over WebTransport, which needed a QUIC listener and a
certificate mounted into the container. All of that is gone: there is one
listener, on loopback, and one way in.
</Note>
...
**Options:**
- `--port` - Box HTTP server port (default: 9000)

The web interface is a dependency-free HTML5 oscilloscope display served from the
box at `http://<box>:9000/scope`. Waveforms arrive over a WebSocket as LSCP
binary frames and are decoded in the browser. The controls are built from the
capabilities detected on the attached unit, so a 2-channel scope does not offer a
channel C, and the page includes a command console that accepts the same
vocabulary as this CLI.
...
- Measurements work on both instruments; `--display` and `--cursor` are Rigol only, since they draw on the instrument's own screen
- Cursor commands are only available for Rigol devices. For a PicoScope, use the cursors in the web UI, which measure client-side over the captured samples
- Edge trigger is the only trigger type supported on both PicoScope and Rigol
- Web visualization requires port 9000 to be accessible on the Lager Box
