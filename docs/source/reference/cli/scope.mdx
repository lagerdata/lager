...
captures a block and computes the value from the samples, because no PicoTech API
has an equivalent of `:MEAS:VPP?`. The consequence is that `--display` and
`--cursor` are Rigol only: they put the reading on the instrument's own screen,
and a PicoScope does not have one.
</Note>
...
| Cursor control | Manual XY cursors | In the web UI, drawn client-side |
| Autoscale | Yes | No |
