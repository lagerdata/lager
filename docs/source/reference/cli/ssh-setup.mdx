...
2. **Generates** the key pair `~/.ssh/lager_box` (and `lager_box.pub`) if it does
   not already exist.
3. **Skips the copy if already authorized** — it asks the box whether the
   `lager_box` public key is already in `~/.ssh/authorized_keys`. It asks with
   `lager_box` first, then with your default keys. If the key is present, it
   goes straight to step 6 (the command is idempotent).
4. **Copies** the public key to the box with `ssh-copy-id -f` (one password prompt).
5. **Verifies** that the key is now in `~/.ssh/authorized_keys`. It reports an
   error only when the box confirms that the key is absent. If the check cannot
   reach the box, the command continues.
...
| `ssh-copy-id reported success but the key is not in the box's authorized_keys.` | The copy did not reach `~/.ssh/authorized_keys`, for example because of a full disk or a read-only home directory | Check the disk and the home directory on the box, then append the key manually (see below) |
