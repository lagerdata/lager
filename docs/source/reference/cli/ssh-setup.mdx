...
appends to `~/.ssh/authorized_keys` outside every marker block, and while
`start_box.sh` preserves such lines against its own rebuild, it cannot preserve
them against another key manager that rebuilds the file from its own source —
that drops every line outside its own markers, and `start_box.sh` then
...
deliberately — a writable key directory would let any user on the box authorize
any key — and the `sudo -n` fallback covers it wherever the box user has a
...
decided the directory should be root-owned, so it is the right place to decide
who may write there. Add it through that fleet's own provisioning:
...
directory. It does not restrict what key is written, since the content is the
box user's to choose — on a box where that user is already root-equivalent that
changes nothing, and on one where it is not, read it before adding it.
