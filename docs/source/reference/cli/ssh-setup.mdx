...
directory comes back; one that was only appended does not. If registration fails
(for example on a box where `/etc/lager` is not writable by the login user) the
command warns and still reports success — the key works — and re-running
`lager ssh-setup` later retries the registration with no password prompt.
