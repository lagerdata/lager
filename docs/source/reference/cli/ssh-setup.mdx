...
Because `-i` replaces SSH's default identity list rather than adding to it,
`lager ssh` also names each of your own default keys that exists (`id_rsa`,
`id_ecdsa`, `id_ed25519`, and their `-sk` forms) after `lager_box`, so a box you
authorized with your own key still connects even when `lager_box` is not
authorized there.
