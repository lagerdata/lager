...
The MCP server reads the bench config at startup. To make agents see your
changes, take one of two actions. Have a connected agent call the `box_manage`
tool with `action="reload"`, which re-reads `/etc/lager/bench.json` and rebuilds
the capability graph. Or restart the box service.
