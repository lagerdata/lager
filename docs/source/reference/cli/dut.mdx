...
`lager dut` manages the **DUT context** stored in `/etc/lager/bench.json` on
...
| Option | Description |
|--------|-------------|
| `--box TEXT` | Lager Box name or IP address (on `show`, `edit` and `add-doc`) |
...
| `--external-id TEXT` | ID of the document in an external document store. |
| `--external-url TEXT` | URL of the document in an external document store. |
...
The MCP server watches `/etc/lager/bench.json`, `/etc/lager/saved_nets.json` and
`/etc/lager/box_id`. When one of them changes on disk, the server reloads on the next
request, so agents see your changes with no manual step.

To force a reload at once, have a connected agent call the `box_manage` tool with
`action="reload"`. You can also restart the box service.

A control plane with a DUT editor writes the same block through `PUT /dut` on the
box's port 9000, which `/status` advertises as `capabilities.dutSync`. The
[bench manifest](/source/reference/cli/bench) reports `dut_updated_at`: the
later of the clock a writer records and the file's mtime. Both sides can then
tell which copy is newer. `lager dut edit` and `add-doc` record that clock.
