...
- `--user TEXT` - Username recorded on box locks. Use the same name as other
  tools that lock boxes. A lock counts as yours only when both tools write the
  identical holder string. See [Box Locking](/source/reference/cli/locking).
  The `LAGER_USER` environment variable overrides this default.
...
With no defaults set, the command prints
`No defaults are currently set. Add one with: lager defaults add`. Running
`lager defaults` with no subcommand gives the same output as `list`.
...
Defaults are stored in the global `.lager` file, `~/.lager`. If
`LAGER_CONFIG_FILE_DIR` is set, the file is `.lager` in that directory. The file
is JSON, and the defaults are under the `DEFAULTS` key:
...
`gateway_id` holds the default box. Each net default uses a `net_` key, for
example `net_debug` or `net_uart`. A `.lager` file in INI format is refused with
an error.
...
- Defaults are global to your user account (stored in `~/.lager`)
