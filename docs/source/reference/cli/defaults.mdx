...
- `--user TEXT` - Username recorded on box locks. Use the same name you use
  in other tools that lock boxes (for example a control plane's display
  name) so a lock taken from either side is recognised by both.
...
```ini
[LAGER]
default_lager_box = my-lager-box
