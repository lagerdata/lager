...
```bash
lager ssh-setup --box <name-or-ip>
```

That prompts for the box password **once**, installs the key, and every later
step of the install runs unattended. Install does not offer a password fallback
of its own: a box configured with `PasswordAuthentication no` never receives
