...
| PicoScope 7 SDK | PicoTech USB drivers for every current PicoScope series (`libps2000` through `libps6000a`), plus the PicoScope 7 application. Installed from PicoTech's apt repository onto the Ubuntu host at `/opt/picoscope/lib` (optional, skipped with `--skip-picoscope`). USB access is granted for the whole PicoTech vendor ID. `lager scope` currently drives 2204A/2205A through the box daemon; other models have drivers and permissions in place |
