...
Paths in `volumes` may use `~`, environment variables, and `${PROJECT_ROOT}` (the directory containing `.lager`) so a committed `.lager` stays portable across machines — e.g. `"${PROJECT_ROOT}:/workspace"`. CLI flags (`--user`, `--group`, `--network`, `--platform`, `--entrypoint`) take precedence over the config value when both are present.
