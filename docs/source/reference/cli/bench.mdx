...
| `bench.dut_updated_at` | When the DUT context last changed (ISO 8601, UTC): the later of the clock a writer records and the file's mtime. A client that keeps its own copy compares clocks before it overwrites either side. |
...
| `reference_entries` | Those entries, for the keys in `reference_keys` only: `get_pattern`, `methods`, `gotchas`, `example_snippet`. The method list is introspected from the drivers on this box. |
