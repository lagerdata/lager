...
|--------|------|---------|-----------------|
| LabJack T7 | FIO0-FIO7, EIO0-EIO7, CIO0-CIO3, MIO0-MIO2 | 3.3V logic | Streaming (high-speed) |
| LabJack U3 | FIO4-FIO7, EIO0-EIO7, CIO0-CIO3 | | Polling |
...
- `FIO0`-`FIO3` are not offered as `gpio` channels. On a U3-HV they are fixed high-voltage analog inputs. Read them with an `adc` net on `AIN0`-`AIN3`.
...
- `FIO4`-`FIO7` and `EIO0`-`EIO7` are also analog inputs. A `gpio` read sets the pin to digital mode, and an `adc` read sets it back to analog mode.
...
- `--poll-interval` applies to non-streaming drivers (LabJack U3, USB-202, Aardvark, FT232H); ignored by LabJack T7
