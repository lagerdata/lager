...
- `--trigger-on` - Trigger on: `gt` (a pulse wider than `--upper`), `lt` (a pulse narrower than `--lower`), or `gtlt` (a pulse between `--lower` and `--upper`). The condition does not select a polarity. Without the width it needs, the command changes no width condition and prints a note.
...
- `--addr-width` - Address width: `7`, `8` or `10` bits
...
- `--source NET` - Trigger source
- `--level FLOAT` - Trigger level
- `--trigger-on` - Trigger on: `start`, `error` (a framing error), `cerror` (a parity error), `data`
- `--parity` - Parity: `even`, `odd`, `none`
- `--stop-bits` - Stop bits: `1`, `1.5`, `2`
- `--baud INT` - Baud rate (110 to 20000000)
- `--data-width INT` - Data width in bits: `5` to `8`
