...
If the box has no binaries, the list reads `(no binaries installed)`. If the
...
- A new binary is visible in the container at once, through a volume mount. On a
  box where that mount is not active yet, `add` prints
  `Note: A first-time setup needs a container restart.` Run `lager update` on
  that box once
