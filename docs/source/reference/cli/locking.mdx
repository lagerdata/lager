...
|-----------------------|----------------------------------------------------------------------------|
| Dev (your machine)    | OS user, or the name set with `lager defaults add --user`                  |
...
The box compares holder names as plain strings, so a lock is yours on every
surface that uses the same name. If another tool (for example a control
plane dashboard) also locks this box, set `lager defaults add --user` to
the display name you use there; a box you lock from either side is then
recognised as yours by both.
...
The three matrix items each get a unique holder, because
`...GITHUB_JOB=hardware-tests/<runner>:<pid>` differs per item. Each item then
