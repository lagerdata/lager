...
1. **Automatic test lock** — `lager python` reserves the box for the
...
- The old ephemeral *per-CLI-command* lock from 0.12–0.13.3 stays
  removed. The new lock is *per `lager python` test*, not per individual
  CLI invocation, which avoids the supply/gdbserver/ssh leaks that
  motivated the 0.13.4 removal.
