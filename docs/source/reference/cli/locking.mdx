...
Unlike the v0.12–0.13.3 design (which used a shared decorator on every
command and was reverted in v0.13.4), this implementation uses the same
TTL + heartbeat + atexit infrastructure as `lager python`, avoiding the
...
behaviour: an eternal hold, with the old "release with `lager boxes unlock`"
message. It arms the lapse TTL only once the box has confirmed it is
heartbeating, so a newer CLI can never leave a lock expiring underneath a job
