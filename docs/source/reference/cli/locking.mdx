...
surface that uses the same name. If another tool, such as a control plane
dashboard, also locks this box, set `lager defaults add --user` to the
display name you use there. A box you lock from either side is then
recognized as yours by both.
