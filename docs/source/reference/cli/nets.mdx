...
`FT2232H` (2 channels: A, B) and `FT4232H` (4 channels: A, B, C, D) physically expose multiple USB interfaces. Channels A and B have an MPSSE engine; on the FT4232H, C and D do not. That matters per *net type*, not per chip: `debug`, `spi` and `i2c` are MPSSE protocols and are limited to A and B, while `gpio` runs as asynchronous bitbang, needs no MPSSE, and works on all four. The user picks an interface per net.
...
Which channels are legal is enforced per net, not per instrument: asking for C or D on an `i2c` or `spi` net fails at net construction naming the channel, rather than somewhere inside pyftdi. A `gpio` net may use any channel the part has.

Before v0.43.0 all three drivers hardcoded interface A, so a board wiring comms to one channel and control lines to another could not be driven at all.
