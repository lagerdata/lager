...
| `set-openocd-config` | Alias for `set-script --backend openocd` |
| `remove-openocd-config` | Alias for `remove-script --backend openocd` |
| `show-openocd-config` | Alias for `show-script --backend openocd` |
...
### `set-openocd-config` / `remove-openocd-config` / `show-openocd-config`

Aliases for the corresponding `*-script` command with `--backend openocd` pinned. Useful when you want to make the intent explicit in scripts or when you have an ambiguous file and don't want to add `--backend openocd` every time.

```bash
# Equivalent invocations
lager nets set-openocd-config debug1 ./probe.cfg --box my-lager-box
lager nets set-script         debug1 ./probe.cfg --backend openocd --box my-lager-box
```
