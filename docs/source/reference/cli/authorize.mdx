...
lager authorize --box lab-lager-box
...
lager boxes add --name lab-lager-box --ip 100.x.y.z
lager authorize --box lab-lager-box

# Re-running against an already-authorized box is safe (no-op)
lager authorize --box lab-lager-box
# lab-lager-box is already authorized — no password needed.
```
...
| Issue | Cause | Fix |
|-------|-------|-----|
| `ssh-copy-id was not found` | OpenSSH client tools are not installed | Install the OpenSSH client, or append the key manually: `cat ~/.ssh/lager_box.pub \| ssh <user>@<box> 'mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys'` |
