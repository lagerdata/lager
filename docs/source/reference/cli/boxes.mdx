...
`pending`. Each row fills in as its box answers. A countdown shows how long
the outstanding boxes have left:
...
The `locked by` column appears only when a box is locked. A version from a
...
| `pending` | Grey | Shown on a terminal only, while that box has yet to answer |
