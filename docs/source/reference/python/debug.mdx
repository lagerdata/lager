...
# Specify custom RAM search parameters for RTT control block detection
with dbg.rtt(search_addr=0x20020000, search_size=0x4000, chunk_size=0x1000) as rtt:
    data = rtt.read_some(timeout=1.0)
```
...
| `search_addr` | `int` | `0x20000000` | RAM start address for RTT control block search |
| `search_size` | `int` | `0x10000` | Size of RAM region to search for RTT control block |
| `chunk_size` | `int` | `0x1000` | Size of each search chunk |
...
# Customize RTT RAM search parameters if the control block is not
# at the default location (e.g., device with non-standard RAM layout)
with debug.rtt(search_addr=0x20020000, search_size=0x4000, chunk_size=0x1000) as rtt:
    data = rtt.read_some(timeout=1.0)
```
...
**RTT Search Parameters:**

The RTT control block is located by scanning a region of RAM. If your device has a non-standard memory layout, you can customize the search range:

| Parameter | Default | Description |
|-----------|---------|-------------|
| `search_addr` | `0x20000000` | RAM start address for the search |
| `search_size` | `0x10000` | Size of the RAM region to scan |
| `chunk_size` | `0x1000` | Size of each search chunk |
