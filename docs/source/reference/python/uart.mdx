...
| `parity` | `str` | Parity: `'none'`, `'even'`, `'odd'`, `'mark'`, or `'space'`, or pyserial's `'N'`, `'E'`, `'O'`, `'M'`, or `'S'` |
| `stopbits` | `float` | Stop bits (1, 1.5, or 2) |
| `timeout` | `float` | Read timeout in seconds (default 0.1). `None` waits until data arrives. |

**Returns:** `serial.Serial` - Connected pyserial object

<Note>
`connect()` raises `ValueError` for a `parity` value that is not in the table
above.
</Note>
