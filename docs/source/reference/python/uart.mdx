...
| `parity` | `str` | Parity: `'none'`, `'even'`, `'odd'`, `'mark'`, or `'space'` |
| `stopbits` | `float` | Stop bits (1, 1.5, or 2) |

**Returns:** `serial.Serial` - Connected pyserial object

<Note>
`connect()` ignores a `timeout` argument. The port always opens with a read
timeout of 0.1 seconds. To change it, set `ser.timeout` on the returned object.
A `parity` value that is not in the table above, such as `'N'` or `'E'`, gives
no parity.
</Note>

<Note>
`connect()` opens the port exclusively. While a `lager uart` session holds the
net, `connect()` raises `serial.SerialException`. While your script holds the
port, `lager uart` on the same net fails with
`UART device /dev/ttyUSB0 is already in use (locked by another session or the lager uart CLI)`.
Close the port with `ser.close()` to release it.
</Note>
