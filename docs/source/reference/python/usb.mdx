...
port by its own behavior instead: its UART output, or a current measurement.
</Note>
...
for whichever hub the net names, which is what most scripts should use. The
per-backend classes are also importable directly, for code that needs to name one:
...
<Note>
Prefer `Net.get`. Constructing a backend class directly ties the script to one
model of hub, so moving the test to a bench with a different hub means editing
code rather than the net record.
</Note>
