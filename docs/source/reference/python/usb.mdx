...
Power-cycle the port: off, wait, on. After power returns, `cycle` waits up to 5
seconds for the device. The return value tells you what the box saw:

- `True` — the device re-enumerated.
- `False` — the device did not come back within 5 seconds.
- `None` — no device was on the port. On an Acroname or YKUSH hub, `None` also
  means that the box did not read its USB topology.

On an Acroname or YKUSH hub, the box compares the kernel USB topology before the
power cut and while the port is off. On a Plugable dock, it reads the connect
status that the hub reports for the port.
...
limited to 0.5-10 seconds. A value outside that range raises `PortStateError`
before the port is switched. **Too short an off time is the failure that matters.**
...
Prefer this over a hand-rolled `disable`/`sleep`/`enable`. It gives these
guarantees that the hand-rolled form does not:

- It restores power on every failure path, so an exception partway through cannot
  leave a port stranded.
- It reports whether the device came back.
- On a Plugable dock, it holds the hub for the whole sequence, so nothing else
  can switch the port while it is dark. On an Acroname or YKUSH hub, another
  process can switch the port during the off time.

<Note>
On a Plugable dock, `cycle` returns on the **hub's** reconnect signal, a few
hundred milliseconds after power returns — not on Linux finishing enumeration.
So `/dev/ttyUSB*` can be absent when it returns, and a `/sys` read taken
immediately still shows the pre-cycle device number. On an Acroname or YKUSH hub,
`cycle` returns when the device is back in the kernel USB topology. On every hub,
poll for what you need rather than reading once.
</Note>

<Warning>
**On a Plugable dock, a powered-off port still appears in `lsusb` and keeps its
`/dev/ttyUSB*`.** The hub raises no change notification while a port is
unpowered, so the kernel does not process the disconnect until power returns.
Never check for a device's absence to decide whether a port is off — use
...
- On a Plugable dock, a port that is powered off still appears in `lsusb` and
  keeps its device nodes. Never use device presence to test whether a port is off
