...
```bash
# whole run, via the dedicated flag (applies to pause() calls with no explicit timeout=)
lager python test.py --box mybox --breakpoint-timeout 1800
lager python test.py --box mybox --breakpoint-timeout 0      # wait indefinitely

# equivalently, via the env var:
lager python test.py --box mybox --env LAGER_BREAKPOINT_TIMEOUT=1800
```

Resolution order is **`timeout=` argument → `--breakpoint-timeout` / `LAGER_BREAKPOINT_TIMEOUT` →
300 s default**, so an explicit `timeout=` in the script wins over the flag/env. (`--breakpoint-timeout`
is just a friendlier spelling of the env var — note `--timeout` is a different setting, the script's
total runtime limit.)
