...
def _choose_driver(instrument: str, netname: Optional[str] = None) -> str:
    ...
    if _T7_RE.search(inst) or inst == "t7":
        return T7
    if inst in ("aardvark_spi", "aardvark", "totalphase_aardvark"):
        return AARDVARK
    if is_ftdi_instrument(inst) or inst == "ft232h_spi":
        return FTDI
    message = helpers.unsupported_instrument_message(ROLE, instrument)
    if netname:
        message = f"Net '{netname}': {message}"
    raise SPIBackendError(message)
...
def _make_driver(rec: Dict[str, Any], overrides: Dict[str, Any] = None):
    ...
    The driver is chosen from the 'instrument' field by _choose_driver.
    """
    netname = rec.get("name", "<unknown>")
