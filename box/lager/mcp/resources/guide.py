...
def register(mcp):
    ...
    def guide_workflow() -> str:
        ...
        return (
            ...
            "\n"
            "## 1. Discover\n"
            "Call `get_bench_summary()` to see what hardware is available: nets, "
            "instruments, and capabilities.  For details on a specific net, call "
            "`get_net_details(net_name)`.  Nets with metadata (description, "
            ...
            "\n"
            "## 3. Learn the API\n"
            "Call `get_api_reference(net_type)` for the Python API of the "
            ...
            "adjust the script and re-run.  Use `quick_read(net_name)` for "
