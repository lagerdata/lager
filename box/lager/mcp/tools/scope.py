...
"""MCP tools for oscilloscope control via direct on-box Net API.

Supports Rigol MSO5000-class scopes (via mapper) and PicoScope (via
passthrough delegation).  All methods forward through `Net.get()` →
mapper → hardware service.
"""
...
_METRIC_MAP = {
    "freq": "frequency",
    "frequency": "frequency",
    "vpp": "voltage_peak_to_peak",
    "vrms": "voltage_rms",
    "vmax": "voltage_max",
    "vmin": "voltage_min",
    "vavg": "voltage_average",
    "period": "period",
    ...
    "rise_time": "rise_time",
    "fall_time": "fall_time",
    "overshoot": "voltage_overshoot",
    "preshoot": "voltage_preshoot",
}
...
def scope_enable(net: str) -> str:
    ...
    Net.get(net, type=NetType.Analog).enable()
    return json.dumps({"status": "ok", "net": net, "enabled": True})


@mcp.tool()
def scope_disable(net: str) -> str:
    ...
    Net.get(net, type=NetType.Analog).disable()
    return json.dumps({"status": "ok", "net": net, "enabled": False})


@mcp.tool()
def scope_autoscale(net: str) -> str:
    ...
    Net.get(net, type=NetType.Analog).autoscale()
    return json.dumps({"status": "ok", "net": net, "action": "autoscale"})


# ── Capture control ──────────────────────────────────────────────────

@mcp.tool()
def scope_start(net: str, single: bool = False) -> str:
    """Start oscilloscope acquisition.

    Args:
        net: Scope net name (e.g., 'scope1')
        single: Capture a single trigger event then stop (default: false)
    """
    from lager import Net, NetType

    scope = Net.get(net, type=NetType.Analog)
    if single:
...
def scope_stop(net: str) -> str:
    ...
    Net.get(net, type=NetType.Analog).stop_capture()
    return json.dumps({"status": "ok", "net": net, "action": "stop"})


@mcp.tool()
def scope_force(net: str) -> str:
    ...
    Net.get(net, type=NetType.Analog).force_trigger()
    return json.dumps({"status": "ok", "net": net, "action": "force_trigger"})
...
def scope_configure(
    ...
    Args:
        ...
        time_per_div: Horizontal timebase in seconds per division
        coupling: Input coupling — 'dc', 'ac', or 'gnd'
        probe: Probe attenuation — '1x', '10x', '100x', or '1000x'
    """
    from lager import Net, NetType
    ...
    if volts_per_div is not None:
        scope.trace_settings.set_volts_per_div(volts_per_div)
        applied["volts_per_div"] = volts_per_div
    if time_per_div is not None:
        scope.trace_settings.set_time_per_div(time_per_div)
        applied["time_per_div"] = time_per_div
    if coupling is not None:
        scope.set_channel_coupling(coupling.upper())
        applied["coupling"] = coupling
    if probe is not None:
        ratio = probe.replace("x", "").replace("X", "")
        scope.set_channel_probe(float(ratio))
        applied["probe"] = probe
    return json.dumps({"status": "ok", "net": net, **applied})


# ── Measurements ─────────────────────────────────────────────────────

@mcp.tool()
def scope_measure(net: str, metric: str, display: bool = False) -> str:
    """Take a measurement on an oscilloscope channel.

    Supported metrics: freq, vpp, vrms, vmax, vmin, vavg, period,
    pw_pos, pw_neg, dc_pos, dc_neg, rise_time, fall_time,
    overshoot, preshoot.
    ...
    scope = Net.get(net, type=NetType.Analog)
    value = getattr(scope.measurement, method_name)(
...
def scope_trigger_edge(
    ...
    Args:
        ...
        coupling: Trigger coupling — 'dc', 'ac', 'low_freq_rej', or 'high_freq_rej' (default: dc)
    """
    from lager import Net, NetType
...
def scope_trigger_pulse(
    ...
    """Configure oscilloscope pulse width trigger.
    ...
    Args:
        ...
        width: Pulse width threshold in seconds (for greater/less conditions)
        upper: Upper width limit in seconds (for range condition)
        lower: Lower width limit in seconds (for range condition)
        mode: Trigger mode — 'normal', 'auto', or 'single' (default: normal)
        coupling: Trigger coupling — 'dc' or 'ac' (default: dc)
    """
    from lager import Net, NetType
...
def scope_trigger_uart(
    ...
    """Configure UART protocol trigger on oscilloscope (Rigol).
    ...
    Args:
        ...
        data: Data pattern to match (hex string, for trigger_on='data')
...
def scope_trigger_i2c(
    ...
    """Configure I2C protocol trigger on oscilloscope (Rigol).
...
def scope_trigger_spi(
    ...
    """Configure SPI protocol trigger on oscilloscope (Rigol).
    ...
    Args:
        ...
        clk_edge: Clock sampling edge — 'positive' or 'negative' (default: positive)
        data: Data pattern to match (hex string)
        data_bits: Data width in bits (default: 8)
        timeout: Timeout value in seconds (for trigger_on='timeout')
        mode: Trigger mode (default: normal)
        coupling: Trigger coupling (default: dc)
    """
    from lager import Net, NetType
...
@mcp.tool()
def scope_cursor_set(net: str, cursor: str, x: float = None, y: float = None) -> str:
    """Set an oscilloscope cursor to an absolute position.

    Args:
        net: Scope net name (e.g., 'scope1')
        cursor: Which cursor — 'a' or 'b'
        x: X-axis position (time)
        y: Y-axis position (voltage)
    """
    from lager import Net, NetType

    scope = Net.get(net, type=NetType.Analog)
    fn = {"a": scope.cursor.set_a, "b": scope.cursor.set_b}[cursor.lower()]
    fn(x=x, y=y)
    return json.dumps({"status": "ok", "net": net, "cursor": cursor, "x": x, "y": y})


@mcp.tool()
def scope_cursor_move(net: str, cursor: str, x: float = None, y: float = None) -> str:
    """Move an oscilloscope cursor by a relative offset.
    ...
    scope = Net.get(net, type=NetType.Analog)
    fn = {"a": scope.cursor.move_a, "b": scope.cursor.move_b}[cursor.lower()]
    fn(x_del=x, y_del=y)
    return json.dumps({"status": "ok", "net": net, "cursor": cursor, "dx": x, "dy": y})


@mcp.tool()
def scope_cursor_hide(net: str) -> str:
    """Hide oscilloscope cursor display.
    ...
    Net.get(net, type=NetType.Analog).cursor.hide()
    return json.dumps({"status": "ok", "net": net, "action": "cursor_hide"})


@mcp.tool()
def scope_cursor_read(net: str) -> str:
    """Read current cursor positions, deltas, and frequency.
    ...
    c = Net.get(net, type=NetType.Analog).cursor
    result = {"status": "ok", "net": net}
    for attr in ("a_x", "a_y", "b_x", "b_y", "x_delta", "y_delta", "frequency"):
        try:
            result[attr] = getattr(c, attr)()
        except Exception:
            pass
