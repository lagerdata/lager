...
allowed. Soft-fails on missing config or empty npm_packages so the
...
_spec = importlib.util.spec_from_file_location("lager_box_config_npm_renderer_cfg", _CONFIG_PY)
...
def _render(argv: list) -> int:
    ...
    body = _HEADER + "".join(f"{p}\n" for p in sorted(c.npm_packages))
